...
- BlockFS persistence with checksum + journal + corruption detection (flat table, directory paths), with mount-time sanitize for duplicates/invalid entries, mode scrubbing, and stale slot zeroing; journal write checks; remove resets entry metadata and zeroes slot data; new files zero their slots before use.
...
- Raw Ethernet/IP/TCP backend for AF_INET sockets when `RSE_NET_RAW=1` (SYN/SYN-ACK/ACK, data, FIN), with retransmit/backoff, FIN retry, window backpressure, ARP cache aging, close sweep, pending handshake cleanup, and local MAC/IP discovery.
...
- User-mode isolation: sys_fork inherits uid/gid + signal handlers; sys_ps hides other users for non-root callers; exec resets signal handlers to default (ignores preserved); sys_kill enforces uid on sig=0 probes; SIGSTOP/SIGCONT stop/resume support (ring3-aware); user-mode signal handlers now delivered via trampoline; kernel-mode custom handlers for cooperative tasks; sys_wait blocks on child exit with wakeup on zombie.
