...
- /dev/net0 UDP loopback plus raw Ethernet/IP/TCP backend for AF_INET sockets when `RSE_NET_RAW=1` (retransmit/backoff, FIN retry, window backpressure, recv buffer backpressure with window update on drain, ARP cache aging).
...
- Pending user signals wake blocked/sleeping targets; wait returns EINTR when a signal is pending.
...
Full system test with raw TCP:
```bash
RSE_NET_RAW_TEST=1 ./scripts/run_full_system_test.sh
```
