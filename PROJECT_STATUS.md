# RSE PROJECT STATUS
Last Updated: January 01, 2026 (PIT timer preemption; ring3 sleep/yield + timeslice; W^X mprotect checks; user-pointer bounds tightened; ring3 maps user-only pages; NET_LITE connect retries/backlog/FIN handling + overflow drops; BlockFS checksum recovery; workload yields; directory listing sort; ISO workflow scripts; logged test heartbeat; quick boot test runner)
...
- TCP-lite framing over `/dev/net0` for loopback handshake/data tests with NET_LITE connect retries/timeouts, FIN-on-close, queued pending accepts, overflow-safe wire buffering, and stricter FIN state handling.
