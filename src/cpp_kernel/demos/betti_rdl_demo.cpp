...
int main() {
  ...
  for (int i = 0; i < 10; i++) {
    kernel.createEdge(i, 0, 0, (i + 1) % 10, 0, 0, 5); // Delay of 5 time units
  }
  ...
  for (int x = 0; x < 4; x++) {
    for (int y = 0; y < 4; y++) {
      for (int z = 0; z < 4; z++) {
        kernel3.createEdge(x, y, z, (x + 1) % 4, y, z, 2); // X direction
        kernel3.createEdge(x, y, z, x, (y + 1) % 4, z, 2); // Y direction
        kernel3.createEdge(x, y, z, x, y, (z + 1) % 4, 2); // Z direction
      }
    }
  }
