...
- Raw Ethernet/IP/TCP backend for AF_INET sockets when `RSE_NET_RAW=1` (SYN/SYN-ACK/ACK, data, FIN), with retransmit/backoff, FIN retry, window backpressure, recv buffer backpressure (no partial accept), window update on drain, ARP cache aging, close sweep, pending handshake cleanup, and local MAC/IP discovery.
