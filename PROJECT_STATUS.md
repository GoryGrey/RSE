...
- Raw Ethernet/IP/TCP backend for AF_INET sockets when `RSE_NET_RAW=1` (SYN/SYN-ACK/ACK, data, FIN), with retransmit/backoff, FIN retry, window backpressure, ARP cache aging, and local MAC/IP discovery.
...
- User-mode isolation: sys_fork inherits uid/gid + signal handlers; sys_ps hides other users for non-root callers.
...
- Recent local runs: `blockfs_test`, `sys_ps_test`, `sys_user_isolation_test` (Jan 02, 2026).
